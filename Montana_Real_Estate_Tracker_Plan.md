...
- **Attribution (added 2026-08-15):** the ArcGIS layer carries an explicit `copyrightText`: "Montana State Library, Department of Revenue" (checked directly via the live API, not assumed). Its own description also notes Ravalli, Silver Bow, Missoula, Flathead, and Yellowstone counties maintain their own parcel data, integrated by the State Library — additional original contributors, not just DOR. This wasn't credited anywhere in the app (only a generic "sourced from Montana's statewide Cadastral API" line existed) until an attribution footer was added covering this, the FRED/Realtor.com citation, and OpenStreetMap in one place.
